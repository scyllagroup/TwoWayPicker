angular.module('umbraco').controller('TwoWayPickerController', 
  function ($scope, dialogService, entityResource, editorState, $log, iconHelper, $routeParams, fileManager, contentEditingHelper, angularHelper, navigationService, $location) {
      ...
      function IsMultiPicker() {
          return $scope.maxNumber != "1";
      }

      function GetFilters() {
          var docTypeAliases = "";
          for (var i = 0; i < $scope.model.config.docTypePropertyMap.length; ++i){
              var selectedDocType = $scope.model.config.docTypePropertyMap[i];
              docTypeAliases = docTypeAliases + "," + selectedDocType.alias;
          }
          docTypeAliases = docTypeAliases.replace(",", "");

          return docTypeAliases;
      }
      ...
      $scope.openContentPicker = function () {
          dialogOptions.filter = GetFilters();
          dialogOptions.multiPicker = IsMultiPicker();
